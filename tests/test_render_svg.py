...
def test_explanation_does_not_grow_svg_width():
    import re
    ...
    assert long_line not in svg


def test_render_svg_keeps_explanation_lines_out_of_svg():
    svg = render_svg(
        (2, 2, 2),
    ...
    assert "Result shape: (2,)" not in svg
